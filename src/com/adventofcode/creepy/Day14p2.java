...
public class Day14p2 {
	...
	public static void main(String[] args) throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(Day14p2.class.getResourceAsStream("/input-day14.txt")));) {
			String line;
		...
		for (int i = 0; i < 2503 ; i++) {
			...
			for (Entry<String, DeerInfo> entry: deers.entrySet()) {				
				...
				if (info.distance > highest) {
					highest = info.distance;
				}
			}
			
			for (Entry<String, DeerInfo> entry: deers.entrySet()) {
				if (entry.getValue().distance == highest) {
